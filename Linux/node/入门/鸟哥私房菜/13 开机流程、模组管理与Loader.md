...
### 磁盘与分区在grub2中的代号  
> grub2主程序安装在MBR中，其主要任务之一就是从磁盘中载入核心文件，让核心能顺利的驱动整个系统，因此，grub2必须能认识磁盘才行，grub2通过不同的代号识别磁盘  
  
grub2对磁盘的代号设定：  
* 磁盘代号以"()"包起来，如：(hd0, 1)  
* 磁盘以hd表示，后面会接一组数字  
* 以搜索顺序作为磁盘的编号  
* 第一个搜索到的磁盘为0号(hd0),第二个搜索到的磁盘为1号(hd1)以此类推  
* 每个磁盘的第一个partition的代号为1，以此类推，如：第一个磁盘的第一个分区表示为"(hd0, 1)"  
* 为了区分分区的不同格式，分区为MBR模式使用"msdos",分区为GPT模式使用"gpt",如：第一个磁盘的第一个MBR模式分区表示为"(hd0, msdos1)",第一个磁盘的第一个GPT模式分区表示为"(hd0, gpt1)"  
### grub2设定档：/etc/default/grub与/etc/grub.d  
> 设定档/boot/grub2/grub.cfg中的内容太复杂，官方不建议我们对其进行修改，我们应该通过/etc/default/grub文件与/etc/grub.d/目录下的文件进行环境的设置  
  
* /etc/default/grub:主要环境设定档  
* /etc/grub.d/*:选单建置脚本  
## initramfs  
> 我们使用initramfs的原因是在开机过程中需要使用到核心模块，但是核心模块放置在"/lib/modules/$(uname -r)/kernel/"中，但这些模块必须在根目录被挂载之后才能被读取，但是核心本身不具备磁盘的驱动程序时，根目录就无法挂载，也就没法取得磁盘驱动，核心模块自然也就无法加载  
  
initramfs能将/lib/modules/中开机过程一定需要的模块打包成一个文件（文件名就是initramfs），然后在开机时通过主机的INT 13硬件功能将该文件读取出来并解压到内存中，然后initramfs会模拟成根目录，因为这个虚拟文件系统(Initial RAM Disk)主要包含磁盘和文件系统的模块，因此核心就能认识实际的磁盘，也就能挂载实际的根目录  
需要initramfs的情况：  
* 根目录所在的磁盘为SATA，USB或SCSI等  
* 根目录所在的文件系统为LVM，RAID等特殊格式  
* 根目录所在的文件系统为非传统Linux认识的文件系统  
* 其他必须要在核心载人时通过的模块  

使用指令dracut/mkinitrd来建立我们需要的特殊initramfs文件  
